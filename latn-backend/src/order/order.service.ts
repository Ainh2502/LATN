    ...
    const order = await this.prisma.$transaction(async (tx) => {
      const newOrder = await tx.order.create({
        data: {
          ...
          payments: {
            create: {
              method: dto.paymentMethod === 'ONLINE' ? 'VNPAY' : 'COD',
              provider: dto.paymentMethod === 'ONLINE' ? 'VNPAY' : 'LATN',
              amount: grandTotal,
...
async cancelOrder(id: string, user: any) {
  ...
  async findAll(user: any) {
    ...
    const isAdmin = user.role === 'ADMIN' || user.role === 'STAFF'|| user.role === 'CUTOMER';
  ...
  async update(id: string, dto: UpdateOrderDto, user: any) {
    const isAdmin = user.role === 'ADMIN' || user.role === 'STAFF' || user.role === 'CUSTOMER';
    if (!isAdmin)
  ...
  // ⭐ Customer cập nhật trạng thái đơn hàng (ví dụ: PENDING → PAID)
  async customerUpdateStatus(id: string, status: string, user: any) {
  ...
  // ❗ Chỉ cho phép Customer sửa đơn của chính họ
  if (order.userId !== user.id) {
    throw new ForbiddenException('Bạn không có quyền thay đổi đơn hàng này');
  }

  // ❗ Chỉ cho đổi trạng thái từ PENDING → PAID (khi thanh toán online xong)
  if (order.paymentStatus !== 'PENDING' || status !== 'PAID') {
    throw new ForbiddenException('Không thể thay đổi trạng thái này');
  }
